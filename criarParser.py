...
def main(model_dir=None): 
    ...
    nlp = spacy.load('pt', tagger=False, parser=False, entity=False, add_vectors=False)

    train_data = [
        (
            ['Eles', 'trocaram', 'algo', '-', 'apoiado', 'seguranças', '.'],
            [1, 1, 4, 4, 5, 1, 1],
            ['nsubj', 'ROOT', 'compound', 'punct', 'nmod', 'dobj', 'punct']
        ),
        (
            ['Eu', 'Gosto', 'Londres', 'e', 'Berlin', '.'],
            [1, 1, 1, 2, 2, 1],
    ...
    doc = Doc(nlp.vocab, words=['Eu', 'gosto', 'seguranças', '.'])
    parser(doc)
    for word in doc:
